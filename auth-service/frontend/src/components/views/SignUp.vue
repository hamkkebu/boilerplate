...
export default defineComponent({
  ...
  setup() {
    ...
    // 로그인 페이지에서 전달받은 값 (존재하지 않는 아이디로 로그인 시도한 경우)
    ...
    const checkIdDuplicate = async () => {
      ...
      try {
        const response = await apiClient.get(
          API_ENDPOINTS.USER_CHECK_DUPLICATE(formData.username)
        );
    ...
    const checkNicknameDuplicate = async () => {
      ...
      try {
        const response = await apiClient.get(
          API_ENDPOINTS.USER_CHECK_NICKNAME_DUPLICATE(formData.nickname)
        );
        // DuplicateCheckResponse: { exists, available, value }
    ...
    const signUpSubmit = async () => {
      ...
      const result = await execute(
        () => apiClient.post(API_ENDPOINTS.USERS, formData),
        {
          onSuccess: () => {
    ...
    // 컴포넌트 마운트 시 자동으로 아이디 중복 확인 (로그인 페이지에서 넘어온 경우)
    onMounted(() => {
      if (prefilledUsername && prefilledUsername.trim().length >= 3) {
        checkIdDuplicate();
      }
    });
