...
public class AuthService {
    ...
    public LoginResponse login(LoginRequest request) {
        ...
        Sample sample = sampleRepository.findBySampleIdAndIsDeletedFalse(request.getUsername())
            .orElse(null);

        boolean userExists = (sample != null);
        boolean passwordMatches = false;

        if (userExists) {
            // 실제 사용자: 실제 비밀번호 검증
            passwordMatches = passwordEncoder.matches(request.getPassword(), sample.getSamplePassword());
        } else {
        ...
        String role = sample.getRole().getAuthority();
        String accessToken = jwtTokenProvider.createAccessToken(sample.getSampleId(), role);
        String refreshToken = jwtTokenProvider.createRefreshToken(sample.getSampleId(), role);
        ...
        SampleResponse responseSample = sampleMapper.toDto(sample);
    ...
    public TokenResponse refresh(String refreshToken) {
        ...
        // 사용자 존재 여부 확인
        Sample sample = sampleRepository.findBySampleIdAndIsDeletedFalse(userId)
            .orElseThrow(() -> {
