...
public class SampleController {
    ...
    @PostMapping
    public ApiResponse<SampleResponse> create(@Valid @RequestBody SampleRequest requestDto) {
    ...
    public ApiResponse<DuplicateCheckResponse> checkIdDuplicate(
    ...
    public ApiResponse<DuplicateCheckResponse> checkNicknameDuplicate(
    ...
    @GetMapping("/{sampleId}")
    public ApiResponse<SampleResponse> getInfo(
            @PathVariable @Size(min = 3, max = 20, message = "sampleId는 3자 이상 20자 이하여야 합니다") String sampleId,
    ...
    @PreAuthorize("hasRole('ROLE_ADMIN')")
    @GetMapping("/page")
    public ApiResponse<PageResponseDto<SampleResponse>> getAllWithPaging(PageRequestDto pageRequest) {
